...
public class BingInterface {
	...
	public synchronized JSONObject queryBing(String query) throws Exception {
		return queryBing(query, 1, BING_RETRY);
	}

	public synchronized JSONObject queryBing(String query, int top) throws Exception {
		return queryBing(query, top, BING_RETRY);
	}

	private synchronized JSONObject queryBing(String query, int top, int retryLeft) throws Exception {
		...
		URL url = new URL(
				"https://api.datamarket.azure.com/Bing/Search/v1/Composite?Sources=%27web%2Bspell%2BRelatedSearch%27&Query=%27"
						...
						+ "%27&Options=%27EnableHighlighting%27&Adult=%27Off%27&$format=Json"
						+ "&$top="+top);
